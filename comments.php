...
<div id="comments" class="my-8 comments-area">
	...
	<?php if ( have_comments() ) : ?>
		<h2 class="comments-title">
			<?php
				printf(
					_nx( 'One comment', '%1$s comments', get_comments_number(), 'comments title', 'portfolio' ),
	...
	<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : ?>
		...
		<nav class="comment-navigation" id="comment-nav-above">
			...
			<h1 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'portfolio' ); ?></h1>

			<?php if ( get_previous_comments_link() ) { ?>
					<div class="nav-previous">
						<?php previous_comments_link( __( '&larr; Older Comments', 'portfolio' ) ); ?>
					</div>
			<?php } ?>

			<?php if ( get_next_comments_link() ) { ?>
				<div class="nav-next">
					<?php next_comments_link( __( 'Newer Comments &rarr;', 'portfolio' ) ); ?>
				</div>
			<?php } ?>
