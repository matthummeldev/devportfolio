...
<section class="bg-white dark:bg-gray-900">
  <div class="max-w-screen-xl px-4 py-8 mx-auto lg:py-16 lg:px-6">
      ...
      <h2 class="mb-4 text-4xl font-extrabold tracking-tight uppercase text-gradient dark:text-white">
            <?php _e('Latest Projects', 'portfolio') ?>
          </h2>
      </div> 
...
<?php
    ...
    if($project_categories){
        foreach($project_categories as $project_category) {          
            echo '<div class="inline-block"><span class="inline-block"><span class="px-3 py-2 mr-2 text-sm font-small leading-[3em] text-white bg-indigo-800 rounded-full dark:bg-gray-700" style="width: max-content">' . $project_category->name . '</a></span></div>';
      }
    } else {
      echo 'No Category';
    }
...
<section class="bg-white dark:bg-gray-900">
  <div class="max-w-screen-xl px-4 py-8 mx-auto lg:py-16 lg:px-6">
      <div class="mx-auto mb-8 lg:mb-16">
            <div class="mx-auto mb-8 lg:mb-16">
            <h2 class="mb-4 text-4xl font-extrabold tracking-tight text-center uppercase text-gradient dark:text-white">
                  <?php _e('Latest Post', 'portfolio') ?>
                </h2>
            </div> 
      <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
             <?php
            ...
            <article class="bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
                <?php 
                  if (has_post_thumbnail()) {
                    ...
                    echo '<img src="'. esc_url($image_url) .'" alt="'. the_title_attribute('echo=0') .'" class="mb-2 rounded-t-lg">';
                    } else {
                ...
                <div class="px-4 py-2">
                  <header class="mb-4 entry-header">
                      <?php the_title( sprintf( '<h3 class="my-6 text-2xl font-bold tracking-tight text-gradient hover:bg-gray-900 dark:text-white">
                       <a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h3>' ); ?>
                      <span class="text-sm"><?php echo 'Posted ', human_time_diff(get_the_time('U'), current_time('timestamp')) . ' ago'; ?></span>
                  </header>
                  <hr class="mb-4"/>
